 ...
 *
 * This repository publishes one package, from the root. The shape is still a
 * list so the rules below, and the monorepo they came from, stay identical.
 *
 * @param {string} root - The repository root.
 * @returns {object[]} A single `{ dir, manifest }` entry, or none.
 */
export function readPackages(root) {
  const file = path.join(root, 'package.json')
  if (!fs.existsSync(file)) return []
  return [{ dir: root, manifest: JSON.parse(fs.readFileSync(file, 'utf8')) }]
}
  ...
  for (const { dir, manifest } of published) {
    ...
    for (const field of DEPENDENCY_FIELDS) {
      for (const [dep, range] of Object.entries(manifest[field] || {})) {
        if (UNPUBLISHABLE.test(range)) {
          problems.push(
            `${name}: ${field}.${dep} is "${range}", which does not resolve from npm.`
          )
          continue
        }
        ...
        if (sibling.private) {
          problems.push(
            `${name}: ${field}.${dep} names a private package, so an install from npm cannot resolve it.`
          )
        } else if (
          !semver.satisfies(sibling.version, range, { includePrerelease: true })
        ) {
          problems.push(
            `${name}: ${field}.${dep} is "${range}", but this release carries ${dep}@${sibling.version}.`
          )
        }
      }
    }
    ...
    if (record && !record.versions.includes(version)) {
      ...
      if (semver.prerelease(version) && !semver.gt(base, record.latest)) {
        problems.push(
          `${name}: ${version} sorts below the published ${record.latest}. Raise the version in package.json to the published one.`
        )
      } else if (
        !semver.prerelease(version) &&
        !semver.gt(version, record.latest)
      ) {
        problems.push(
          `${name}: ${version} is not above the published ${record.latest}.`
        )
      }
    }
    ...
    if (files) {
      for (const entry of manifest.files || []) {
        ...
        if (!fs.existsSync(target)) {
          problems.push(
            `${name}: files entry "${entry}" does not exist. Run yarn build first.`
          )
        } else if (
...
export function fetchRecord(name) {
  ...
  return new Promise((resolve, reject) => {
    const request = https.get(
      ...
      (response) => {
        ...
        if (response.statusCode !== 200) {
          ...
          return reject(
            new Error(
              `npm registry answered ${response.statusCode} for ${name}.`
            )
          )
        }
    ...
    request.on('timeout', () =>
      request.destroy(
        new Error(
          `npm registry did not answer for ${name} within ${REGISTRY_TIMEOUT / 1000}s.`
        )
      )
    )
...
async function main() {
  ...
  const problems = checkPackages({
    packages,
    registry,
    files: !args.includes('--skip-files'),
  })
  ...
  if (problems.length) {
    console.error(
      `release-check: ${problems.length} problem(s) across ${count} published package(s).`
    )
    process.exit(1)
  }
