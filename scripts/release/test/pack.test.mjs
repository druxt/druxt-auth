...
const names = (packages) =>
  publishOrder(packages).map(({ manifest }) => manifest.name)

test('a package follows the siblings it depends on', () => {
  const packages = [
    pkg('site', { core: '1.0.0', router: '1.0.0' }),
    pkg('router', { core: '1.0.0' }),
    pkg('core'),
  ]
...
test('peer dependencies order packages too', () => {
  const packages = [
    pkg('blocks', {}, { peerDependencies: { core: '1.0.0' } }),
    pkg('core'),
  ]
