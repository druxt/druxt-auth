...
test('a coherent set has no problems', () => {
  const packages = [
    core(),
    pkg({ name: 'site', version: '2.0.0', dependencies: { core: '^1.2.0' } }),
  ]
...
test('a specifier that only resolves in the repository is refused', () => {
  for (const range of [
    ...
    const packages = [
      core(),
      pkg({ name: 'site', version: '2.0.0', dependencies: { core: range } }),
    ]
...
test('an internal range that excludes the sibling in this release is refused', () => {
  const packages = [
    core('0.25.0'),
    pkg({ name: 'site', version: '2.0.0', dependencies: { core: '^0.24.0' } }),
  ]
  assert.match(
    checkPackages({ packages, files: false })[0],
    /but this release carries core@0\.25\.0/
  )
})
...
test('an exact pin on a snapshot sibling passes', () => {
  ...
  const packages = [
    ...
    pkg({
      name: 'site',
      version: `0.14.4-dev.20260920003207`,
      peerDependencies: { core: snapshot },
    }),
  ]
  assert.deepEqual(checkPackages({ packages, files: false }), [])
})
...
test('a dependency on a private sibling is refused', () => {
  const packages = [
    pkg({ name: 'utils', version: '1.0.0', private: true }),
    pkg({ name: 'site', version: '2.0.0', dependencies: { utils: '^1.0.0' } }),
  ]
  assert.match(
    checkPackages({ packages, files: false })[0],
    /names a private package/
  )
})
...
test('an invalid version is refused', () => {
  assert.match(
    checkPackages({ packages: [core('1.2')], files: false })[0],
    /not a valid version/
  )
})
...
test('a stable version must rise above the published one', () => {
  ...
  assert.deepEqual(
    checkPackages({ packages: [core('1.2.1')], registry, files: false }),
    []
  )
  assert.match(
    checkPackages({ packages: [core('1.1.5')], registry, files: false })[0],
    /not above the published 1\.2\.0/
  )
})
...
test('an already published version passes, because publish skips it', () => {
  ...
  assert.deepEqual(
    checkPackages({ packages: [core('1.2.0')], registry, files: false }),
    []
  )
})
...
test('a snapshot of an already published version is refused', () => {
  ...
  assert.match(
    checkPackages({
      packages: [core('0.17.3-dev.20260920003207')],
      registry,
      files: false,
    })[0],
    /sorts below the published 0\.17\.3/
  )
  assert.deepEqual(
    checkPackages({
      packages: [core('0.17.4-dev.20260920003207')],
      registry,
      files: false,
    }),
    []
...
test('unpublished versions are listed, private packages left out', () => {
  const packages = [
    core('1.2.1'),
    pkg({ name: 'site', version: '2.0.0' }),
    pkg({ name: 'fresh', version: '0.1.0' }),
    pkg({ name: 'utils', version: '1.0.0', private: true }),
  ]
  const registry = {
    core: { latest: '1.2.0', versions: ['1.2.0'] },
    site: { latest: '2.0.0', versions: ['2.0.0'] },
    fresh: null,
  }
  assert.deepEqual(listUnpublished({ packages, registry }), [
    'core@1.2.1',
    'fresh@0.1.0',
  ])
})
