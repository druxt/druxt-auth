...
export function publishOrder(packages) {
  ...
  const visit = (entry, trail) => {
    ...
    const deps = DEPENDENCY_FIELDS.flatMap((field) =>
      Object.keys(entry.manifest[field] || {})
    )
...
function main() {
  ...
  const known = new Set(
    ordered.map(({ manifest }) => `${manifest.name}@${manifest.version}`)
  )
  const unknown = only.filter((tag) => !known.has(tag))
  if (unknown.length)
    throw new Error(`Not in this release set: ${unknown.join(', ')}`)
  ...
  for (const { dir, manifest: pkg } of ordered) {
    ...
    if (!fs.existsSync(path.join(destination, file)))
      throw new Error(`npm pack did not write ${file}.`)
