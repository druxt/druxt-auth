 ...
 *
 *   node scripts/release/notes.mjs druxt-auth@0.5.0
 */
...
function main() {
  ...
  if (at < 1) throw new Error('Pass a release tag, such as druxt-auth@0.5.0.')

  const [name, version] = [tag.slice(0, at), tag.slice(at + 1)]
  const found = readPackages(process.cwd()).find(
    ({ manifest }) => manifest.name === name
  )
  if (!found) throw new Error(`No package named ${name}.`)

  const notes = extractNotes(
    fs.readFileSync(path.join(found.dir, 'CHANGELOG.md'), 'utf8'),
    version
  )
  if (notes === null)
    throw new Error(`${name} has no changelog section for ${version}.`)
  console.log(notes)
}
