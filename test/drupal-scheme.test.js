...
describe('DrupalScheme', () => {
  beforeEach(() => {
    ...
    $auth = {
      ...
      $storage: {
        setUniversal: jest.fn((key, value) => { storage[key] = value }),
        getUniversal: jest.fn((key) => storage[key]),
        removeUniversal: jest.fn((key) => { delete storage[key] }),
      },
    }
  })

  const scheme = (options = {}) => new DrupalScheme($auth, { name: 'drupal-authorization_code', ...options })

  test('login without credentials is oauth2 login', async () => {
    expect(await scheme().login({ state: 'x' })).toStrictEqual({ oauth2: 'login', options: { state: 'x' } })
    expect($auth.request).not.toHaveBeenCalled()
  })

  test('login with credentials signs in to Drupal first, then starts oauth2', async () => {
    const result = await scheme().login({ credentials: { name: 'editor', pass: 'secret' } })
  ...
  test('a refused sign-in stops before oauth2', async () => {
    $auth.request.mockRejectedValueOnce(httpError(400, 'Sorry, unrecognized username or password.'))
    await expect(scheme().login({ credentials: { name: 'editor', pass: 'wrong' } })).rejects.toThrow('unrecognized')
  })

  test('an existing Drupal session carries on to oauth2', async () => {
    $auth.request.mockRejectedValueOnce(httpError(403, 'This route can only be accessed by anonymous users.'))
    expect(await scheme().login({ credentials: { name: 'editor', pass: 'secret' } })).toStrictEqual({ oauth2: 'login', options: {} })
  })

  test('a blocked account is an error', async () => {
    $auth.request.mockRejectedValueOnce(httpError(403, 'The user has not been activated or is blocked.'))
    await expect(scheme().login({ credentials: { name: 'editor', pass: 'secret' } })).rejects.toThrow('blocked')
  })

  test('endpoints can be overridden', async () => {
    await scheme({ endpoints: { drupalLogin: '/api/user/login?_format=json' } }).login({ credentials: { name: 'a', pass: 'b' } })
    expect($auth.request.mock.calls[0][0].url).toBe('/api/user/login?_format=json')
  })
  ...
  test('resetPassword sends an address as mail and anything else as name', async () => {
    ...
    expect($auth.request.mock.calls[0][0]).toMatchObject({ url: '/user/password?_format=json', data: { mail: 'editor@example.com' } })
    await scheme().resetPassword('editor')
    expect($auth.request.mock.calls[1][0].data).toStrictEqual({ name: 'editor' })
  })
})
