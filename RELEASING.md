...
A push to `develop` with a pending changeset cuts a snapshot. A snapshot version reads `0.4.1-dev.20260925004838`: the version the pending changesets add up to, then the tag and a timestamp. Install it with:
...
4. The push that follows publishes the new version to `latest`. It also pushes a `druxt-auth@version` tag, with a GitHub Release built from that version's changelog section.
