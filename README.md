...
- `drupal-authorization_code`
  ...
  | Setup                                       | Credentials                                                                                                                                                                                                              |
  | ------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
  | Nuxt server proxying Drupal, on any servers | Works, and `druxt: { proxy: { api: true } }` sets it up. The module proxies `/user/login`, `/user/logout`, `/user/password`, `/oauth/authorize` and `/oauth/token`, and points the `authorization` endpoint at the site. |
